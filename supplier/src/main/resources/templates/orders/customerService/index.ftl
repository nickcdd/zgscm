...
<#include "/_head_with_datepicker.ftl" />
    ...
    <meta name="nav-url" content="/customerService/index">
    <script type="text/javascript">
        //全局变量 用于保存单个选择不通过的ordersid
        var singleOrdersSid = "";
        $(function () {
...
//            });
            $("#selectAll").click(function () {
                ...
                if ($("input[name='ordersSid']:checked").length > 0) {
                    ...
                    $("#batchBtn2").prop("disabled", false);
                } else {
                    $("#batchBtn1").prop("disabled", true);
                    $("#batchBtn2").prop("disabled", true);
                }
        ...
        //选择不通过需要填写原因
        function fillReason(isSingle, orderSid) {
            if (isSingle) {
                singleOrdersSid = orderSid;
                $('#singleReasonConfirm').show();
                $('#bathReasonConfirm').hide();
                $('#reasonModal').modal("show");
            } else {
                singleOrdersSid = "";
                $('#singleReasonConfirm').hide();
                $('#bathReasonConfirm').show();
                $('#reasonModal').modal("show");
            }
        }
        //隐藏填写原因模态框
        ...
        function confirmBatchOrders(flagStatus) {
            if (flagStatus) {
                ...
                if ($('#modalReason').val() == null || $('#modalReason').val() == "" || $('#modalReason').val() == undefined) {

                    showInfoFun("原因必须填写", "danger");

                } else {
                    $('#reasonModal').modal("hide");
                    confirmInfoFun("deliverGoods(" + flagStatus + ")", "确认执行不通过操作?", '信息提示', true);
        ...
        function confirmSingleOrders(orderSid, flagStatus) {
            if (flagStatus) {
                ...
                if ($('#modalReason').val() == null || $('#modalReason').val() == "" || $('#modalReason').val() == undefined) {
                    ...
                    $('#reasonModal').modal("hide");
                    confirmInfoFun("singleReviewRefund(" + singleOrdersSid + "," + flagStatus + ")", "确认执行不通过操作?", '信息提示', true);
                }
            }
        }
        /**
...
//            window.location.href = "/orders/deliverMultiGoods/?ordersSids=" + sids;
        ...
        function batchReviewRefund(sids, flag) {
            ...
            $.ajax({
                type: "POST",
                url: "/customerService/batchReviewRefund",
                data: {"ordersSids": sids, "flag": flag, "reason": $('#modalReason').val()},
                dataType: "JSON",
                traditional: true,
                async: false,
...
//            $(obj).attr("disabled","disabled");
            ...
            $.ajax({
                ...
                data: {"ordersSid": orderSid, "flag": flag, "reason": $('#modalReason').val()},
                dataType: "JSON",
                async: false,
                success: function (result) {
                    if (result.success) {
            ...
            <div class="box-body">
                <div class="row placeholders">
                    <div class="col-md-12 placeholders">
                        <form id="infoForm" action="/customerService/index" method="post" class="form-inline"
                              role="search-form">
                            ...
                            <#if orders ??>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <#if orderGood_index == 0>
                                                            ...
                                                            <#elseif order.status == 10>
                                                                <td style="width: 13%; text-align: center;"
                                                                    rowspan="${order.ordersGoodsList?size}">
                                                                    <p class="text-primary">退换货成功</p></td>
                                                            </#if>
                                                            <td style="width: 13%; text-align: center;"
                                                                rowspan="${order.ordersGoodsList?size}">
                                                            ...
                                                            <#--</button>-->
                                                                ...
                                                                <#if order.status == 8 || order.status == 9>
                                                                    <button type="button" class="btn btn-success btn-xs"
                                                                            value="${order.sid?c}"
                                                                            onclick="confirmSingleOrders(${order.sid?c},true)">
                                                                        <i class="fa fa-check"></i> 通过
                                                                    </button>
                                                                    <button type="button" class="btn btn-danger btn-xs"
                                                                            value="${order.sid?c}"
                                                                            onclick="fillReason(true,${order.sid?c})">
                                ...
                                <div class="text-right" style="margin-bottom: 5px">
                                    ...
                                    <button type="button" id="batchBtn2" class="btn-lg btn btn-primary btn-danger"
                                            data-toggle="modal" data-target="#deliverModal"
                                            disabled="disabled" onclick="fillReason(false,null)">选中项不通过
                                    </button>
                                </div>
...
<form id="exportForm" action="/customerService/export" method="post">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row form-group">
                    ...
                    <div class="form-group has-feedback">
                        ...
                        <div class="col-sm-6">
                            <input type="text" class="form-control" id="modalReason" placeholder="不通过时必填原因"
            ...
            <div class="modal-footer">
                <button type="button" style="display: none;" class="btn btn-primary btn-confirm"
                        id="singleReasonConfirm" onclick="confirmSingleOrders(null,false)">确认
                </button>
                <button type="button" style="display: none;" class="btn btn-primary btn-confirm" id="bathReasonConfirm"
                        onclick="confirmBatchOrders(false)">批量提交原因确认
                </button>
