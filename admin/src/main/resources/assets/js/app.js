 ...
 * 用于信息显示确认框
 */
var confirmInfoFun = function (doFun, msg, title, flag) {
    var str = [];
    str.push('<div id="testModel" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true" data-backdrop="static" data-keyboard="false">');
    str.push('<div class="modal-dialog">');
    str.push('<div class="modal-content">');
    str.push('<div class="modal-header">');
    str.push('<h4 class="modal-title" id="myModalLabel">', title, '</h4></div>');
    str.push('<div class="modal-body">');
    ...
    str.push('</div>');
    str.push('<div class="modal-footer">');
    if (doFun != null || doFun != "" || doFun != undefined) {
        str.push('<button type="button" id="testBtn" class="btn btn-info" data-dismiss="modal" onclick="', doFun, '">确认</button>');
        if (flag) {
            str.push('<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>');
        }

    } else {
        str.push('<button type="button" id="testBtn" class="btn btn-info" data-dismiss="modal">确认</button>');
    }
    // str.push('<a href="', url, '" class="btn btn-primary">是</a>');
    str.push('</div></div></div></div>');
