...
@Service
@Transactional
public class UploadSupplierGoodsExcelServiceImpl implements UploadSupplierGoodsExcelService {
    //上传路径
    @Value("${upload_path}")
    ...
    @Override
    public HashMap<String,Object> uploadExcelFile(MultipartFile file, Supplier supplier) {
        ...
        try {
            ...
            //新图片名称
            String newFileName =new SimpleDateFormat("yyyyMMddHHmmss").format(new Date())+"_"+supplier.getSid()+"_"+fileOriginalName.substring(fileOriginalName.lastIndexOf("."));
            File   dir         = new File(uploadPath + "/supplierGoodsExcel");
            if (!dir.exists()) {
                dir.mkdirs();
            }
            ...
            resultMap=this.readExcel(readFile,supplier);
            return resultMap;
        }catch(Exception e){
            e.printStackTrace();
            flag=false;
    ...
    public boolean createExcelAndSaveFile(List<GoodsDto> list, String path) throws IOException {
        ...
        try {
            File dir = new File(uploadPath + "/supplierResultExcel");
            if (!dir.exists()) {
                dir.mkdirs();
            }
            path=uploadPath+"/supplierResultExcel/"+path;
    ...
    public HashMap<String,Object> readExcel(File readFile,Supplier supplier){
