...
function getGooodsCategory(sid) {
    ...
    $.getJSON('detailGoodsCategory?sid=' + sid, function (result) {
        if (result.success) {
            $form.clearForm(true);
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data.record, function (i, item) {
                    str += "<li ><a href='#' data-info=" + item.sid + " data-flag='1' onclick='findGoodsCategory(this)'><i class='fa fa-circle-o'></i> <span>" + item.cname + "</span></a></li>";
