...
public class RSAUtils {
     ...
     * @return
     */
    public static String urlEncryptByPublic(String encryptingStr, String publicKeyStr) {
        try {
            // 将公钥由字符串转为UTF-8格式的字节数组
            byte[] publicKeyBytes = decryptBase64(publicKeyStr);
            // 获得公钥
            ...
            cipher.init(Cipher.ENCRYPT_MODE, publicKey);
            // 返回加密后由Base64编码的加密信息
            return safeUrlBase64Encode(cipher.doFinal(data));
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     * @return
     */
    public static String urlDecryptByPrivate(String encryptedStr, String privateKeyStr) {
        try {
            // 对私钥解密
            byte[] privateKeyBytes = decryptBase64(privateKeyStr);
            // 获得私钥
            PKCS8EncodedKeySpec keySpec = new PKCS8EncodedKeySpec(privateKeyBytes);
            // 获得待解密数据
            byte[]     data       = safeUrlBase64Decode(encryptedStr);
            KeyFactory factory    = KeyFactory.getInstance(KEY_RSA);
            PrivateKey privateKey = factory.generatePrivate(keySpec);
            // 对数据解密
            Cipher cipher = Cipher.getInstance(factory.getAlgorithm());
    ...
    public static byte[] safeUrlBase64Decode(final String safeBase64Str) throws IOException {
        ...
        return Base64.getUrlDecoder().decode(safeBase64Str);
